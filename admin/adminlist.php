...
/*	if($_GET["act"]=="search" && $_POST['title']!='')
    ...
    <tr height="35px">
		...
		<td><?php echo $row['logintimes'];?> 次</td>
		
		<td><?php echo $rowcc['ccc'];?>&nbsp;<?php if($rowcc['ccc']>0){?><a href="userlist.php?act=search&title=<?php echo $row['username'];?>" class="tablelink"></a><?php }?></td>
		<td><?php echo $row['lasttime'];?></td>
		<td><a href="?act=repass&id=<?php echo $row['id'];?>" alt="重置为默认密码123456" title="重置为默认密码123456">重置密码</a></td>

		<td><a href="adminlist.php?id=<?php echo $row['id'];?>&act=del" onclick="if (confirm('确定要删除吗？')) return true; else return false;" class="tablelink"> 删除</a></td>
    </tr>
    <?php
	}
	?>    
