...
<body>
    ...
    <tr height="35px">
		...
		<td><?php echo $row['anums'];?></td>
		
		<td><?php echo $rowcc['ccc'];?>&nbsp;<?php if($rowcc['ccc']>0){?><a href="artile.php?userid=<?php echo $row['userid'];?>" class="tablelink">(查看文章)</a><?php }?></td>
		<td><?php echo $row['qq'];?></td>
		<td><a href="?act=repass&id=<?php echo $row['id'];?>" alt="重置为默认密码123456" title="重置为默认密码123456">重置密码</a></td>
...
?>
			...
			<td><?php echo $row['shuyu'];?></td>
	<td><?php echo $row['beizhu2'];?></td>
		<td><a href="useredit.php?id=<?php echo $row['id'];?>" class="tablelink">查看</a>     <a href="userlist.php?id=<?php echo $row['id'];?>&act=del" onclick="if (confirm('确定要删除吗？')) return true; else return false;" class="tablelink"> 删除</a></td>
    </tr>
    <?php
	}
	?>    
