...
function delall(){
	...
	if(a>0){
		if(confirm('您已选中'+a+'条数据,是否确认删除?')){
			document.forms.action='artile.php?act=delall&t=<?php echo $_GET["t"];?>&page=<?php echo $_GET["page"];?>&s=<?php echo $_GET["s"];?>';	
...
function shall(v){
	...
	if(a>0){
		if(confirm('您已选中'+a+'条数据,是否确认审核?')){
			document.forms.action='bdlist.php?act=shall&v='+v+'&t=<?php echo $_GET["t"];?>&page=<?php echo $_GET["page"];?>&s=<?php echo $_GET["s"];?>';	
			document.getElementById('forms').submit();
		}else{
			return false;
		}
...
<body>
	...
	<li><a href=""><?php echo $types[$_GET['t']];?>管理</a></li>
    </ul>
    </div>
	...
	<div class="tools">
		...
		<li style="float:right">
			<form name="searchsoft" method="post" action="?act=search&t=<?php echo $_GET['t'];?>&s=<?php echo $_GET['s'];?>">
			<strong>&nbsp;&nbsp;&nbsp;搜索关键字：</strong>
	...
	<td><input name='ids[]' type='checkbox' onClick="unselectall()" id="ID" value='<?php echo $row['id'];?>'></td>
	<td><a href="../view.php?fid=<?php echo $row['infoid'];?>" target="_blank"><?php echo $row['title'];?></a></td>
	<td><?php echo $row['userid'];?></td>
    <td><a href="<?php echo $row['adlink'];?>" target="_blank"><img src="../<?php echo $row['adpic'];?>" width="260" style="max-height:60px;"></a></td>
	<td><?php echo $row['wcount'];?></td>
	<td><?php echo $row['acount'];?></td>
    <td>
	<a href="artile.php?id=<?php echo $row['id'];?>&act=del&t=<?php echo $_GET['t'];?>&page=<?php echo $_GET['page'];?>&s=<?php echo $_GET['s'];?>&userid=<?php echo $_GET['userid'];?>" onclick="if (confirm('确定要删除吗？')) return true; else return false;" class="tablelink"> 删除</a>
	</td>
    </tr>
    <?php
	}
